  ...
  return (
    <div>{parts.map((seg, si) => {
      ...
      return <div key={si}>{seg.v.split('\n').map((line, li) => {
        ...
        const bq=line.match(/^> (.+)/)
        if (bq) return <div key={li} style={{ borderLeft:`3px solid ${G}`,paddingLeft:12,marginBottom:4,color:'rgba(255,255,255,0.55)',fontStyle:'italic' }}>{bq[1]}</div>
        const parts2=line.split(/(`[^`]+`|\*\*[\s\S]+?\*\*)/g)
        return <div key={li} style={{ marginBottom:2 }}>{parts2.map((p,i) => {
...
export default function Home() {
  ...
  const send = useCallback(async (txt) => {
    ...
    if (conv?.title === 'New Chat') {
      setConvs(p => p.map(c => c.id === activeId ? { ...c, title: text.slice(0, 36), messages: newMsgs } : c))
    } else {
      setConvs(p => p.map(c => c.id === activeId ? { ...c, messages: newMsgs } : c))
    }
    ...
    try {
      ...
      const aiMsg = { id: Date.now() + 1, role: 'assistant', content: full }
        ...
        <div style={{ flex:1,overflowY:'auto',padding:'4px 8px' }}>
          ...
          {convs.map(c => (
            <div key={c.id} onClick={() => setActiveId(c.id)}
              ...
              <span onClick={e => delConv(c.id,e)} style={{ opacity:0,fontSize:11,padding:'0 3px' }}

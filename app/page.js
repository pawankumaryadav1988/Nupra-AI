...
export default function Home() {
  ...
  const send = useCallback(async (txt) => {
    ...
    if (!activeId) { alert('Please create a new chat first!'); return }
    ...
    if (conv?.title === 'New Chat' && text) {
      ...
      await supabase.from('conversations').update({ title: newTitle, updated_at: new Date().toISOString() }).eq('id', activeId)
      setConvs(p => p.map(c => c.id === activeId ? { ...c, title: newTitle } : c))
    }
    ...
    try {
      const res = await fetch('/api/chat', {
        ...
        body: JSON.stringify({ messages: allMsgs, conversationId: activeId, model }),
      })
